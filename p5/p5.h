...
class sAVL {
    private:
                ...
                int treeCount;
		// The height of that node in relation to its subtree
		int heigh(sNode *p) const;
                // Points to the root of the entire tree
                sNode *root;

		// Calculates the height of the tree based on the height
		// of its children
		int calcHeight(sNode *p);
                ...
                // Clears the tree
                void clear(sNode *&p);
		// rotate the subtree to the left to fix the right-heavy
		// imbalance
